...
class clsDB
{
	 ...
	 */
	public function getFrom($strForeignName, $strField, $strExtraInfo = '')
	{
		...
		if(!clsDB::isValidFieldName($strExtraInfo))
			throw new Exception(ERRORMSG_INVALID);

		$objForeign = $this->getForeignObject($strForeignName, $strExtraInfo);
	...
	/** Retrieve the first element with the given value in the requested field. */
	public static function getByName($strName, $strValue, $strField = 'name')
	{
		...
		$strValue = clsDB::escapeValue($strValue);
		$strField = clsDB::getFieldNameStatic($strName, $strField);

		$arrResults = clsDB::getListStatic($strName, "`$strField`='$strValue'", 'id', ASC, 1);

		if(sizeof($arrResults))
			return $arrResults[0];
		else
			return null;
	}
	...
	public function getFromRequest()
	{
		...
		{
			if(!clsDB::isValidFieldName($key))
				throw new Exception(ERRORMSG_INVALID . ": **$key**");
